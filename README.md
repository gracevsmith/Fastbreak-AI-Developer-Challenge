...
The main challenge that I faced was a lack of data. I made up for this by creating handmade data and scraping the internet for data relevate to sports scheduling. I also added a feedback section to the code, which would essentially create a new Algolia index from the user's prompt and the corrected filled-out template. In this way, the model creates new data the more users interact with it. Note that due to the way data is called in the version of the code I've uploaded to GitHub (called from files rather than making repeated API calls, which is expensive for the API key I have access to), the new data is not integrated. However, the code to integrate the data is functioning, and if one were to load the data through Algolia rather than the premade .pkl datafile, the new data would be incorporated.

A second challenge that I faced was extracting entities with semantic search methods. Ideally I would have lots of pre-labeled data to help train a machine learning method, but using semantic search to extract parameters is a little tricky. I decided to use the information from my Algolia indexes to help map parameter labels to my imported data (e.g. if teams was a parmeter, "Buffalo Bills":team). In this way, when a term is found to be semantically similar to something in my index (e.g. "bills" is semantically similar to "Buffalo Bills"), the Aloglia mapping helps identify which parameter field in the pre-made template should be populated with that term (i.e. "bills" fills in a "team" parameter space).

Another challenge that I faced was uncertaintly which words in an NLP prompt were associated with which labels/ parameters. This was a little difficult to figure out since there weren't any examples of a natural language prompt with extracted parameter outputs. I made educated guesses on the parameters that seems to overlap (rounds/ games). In the future, having some labeled data and further access to/ understanding of the core optimization model would vastly improve the extracted parameter portion of the code.

Overall, the semantic search methods struggle without vast amounts of labeled data stored in the Algolia index. Ways to improve the code include using a neural net (e.g. RoBERTa) instead of semantic search, creating more labeled data (labeling both template type and extracted parameters), and updating the data to include user corrections in a more seemless manner.
